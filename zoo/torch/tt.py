...
def _apply_sinc_resample_kernel(
    ...
    # 使用提供的conv1d函数
    resampled = conv1d(waveform[:, None], kernel, stride=orig_freq)
...
if __name__ == '__main__':
    file = get_testfile('assets', 'texas_assist_sr8000.wav')
    waveform, sr = torchaudio.load(file)
    expected_result = torchaudio.functional.resample(waveform, sr, 16000)
    print(expected_result)
    print(expected_result.dtype, expected_result.shape)

    actual_result = resample(waveform.numpy(), sr, 16000)
    print(actual_result)
    print(actual_result.dtype, actual_result.shape)

    sound = Sound.open(file)
    sound = sound.resample(16000)
    t = torch.from_numpy(sound.to_numpy()[0])
    print(t)
    print(t.dtype, t.shape)

    sound = Sound.open(file)
    ax = resample(sound.to_numpy()[0], sound.sample_rate, 16000)
    print(ax)
    print(ax.dtype, ax.shape)
